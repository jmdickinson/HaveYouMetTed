...
<br />
 <h2 class="commentstitle"><?php comments_number('0 Comments', '1 Comment', '% Comments' );?> to <em>&#8220;<?php the_title(); ?>&#8221;</em></h2>
   ...
   <?php foreach ($comments as $comment) : ?>
      <li class="<?php if($comment->user_id > 1) echo 'user'; else if($comment->user_id == 1) echo 'author'; else echo $oddcomment; ?>" id="comment-<?php comment_ID() ?>">
         ...
         <p><?php comment_author_link() ?>&nbsp;|&nbsp; <?php comment_date() ?> <?php if ($user_ID) { edit_comment_link(__("(e)"), ' | '); } ?></p>
...
<?php if ('open' == $post->comment_status) : ?>
<h2 class="commentstitle formtitle">Leave a Reply</h2>
<p>This is a Gravatar-enabled weblog. If you want your own picture instead of the monster, register for free at <a href="http://gravatar.com" target="_blank">Gravatar</a> with the email address you use to comment.</p>
